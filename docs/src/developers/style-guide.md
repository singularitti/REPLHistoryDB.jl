...
!!! info
    The style guide is always a work in progress, and not all REPLHistoryDB code
    follows the rules. When modifying REPLHistoryDB, please fix the style violations
...
REPLHistoryDB uses [JuliaFormatter](https://github.com/domluna/JuliaFormatter.jl) as
an auto-formatting tool.

We use the options contained in [`.JuliaFormatter.toml`](https://github.com/singularitti/REPLHistoryDB.jl/blob/main/.JuliaFormatter.toml).

To format your code, `cd` to the REPLHistoryDB directory, then run:
...
!!! info
    A continuous integration check verifies that all PRs made to REPLHistoryDB have
