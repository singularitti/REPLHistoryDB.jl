...
[REPLHistoryDB](https://github.com/singularitti/REPLHistoryDB.jl).
...
2. Run the following commands and wait for them to finish:
   ...
   julia> Pkg.add("REPLHistoryDB")
   ```

3. Run

   ```julia-repl
   julia> using REPLHistoryDB
   ```
...
version of REPLHistoryDB, type

```@repl
using Pkg
Pkg.update()
pkg"add https://github.com/singularitti/REPLHistoryDB.jl"
```

in the second step above.

## Update REPLHistoryDB
...
our [GitHub repository](https://github.com/singularitti/REPLHistoryDB.jl)
for new releases.
Once we release a new version, you can update REPLHistoryDB by typing

```@repl
using Pkg
Pkg.update("REPLHistoryDB")
Pkg.gc()
```

in the Julia REPL.

## Uninstall and reinstall REPLHistoryDB
...
1. To uninstall, in a Julia session, run
   ...
   julia> Pkg.rm("REPLHistoryDB")

   julia> Pkg.gc()
   ```

2. Press `ctrl+d` to quit the current session. Start a new Julia session and
   reinstall REPLHistoryDB.
